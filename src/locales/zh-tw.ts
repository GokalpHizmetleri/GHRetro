...
export const translation: ResourceLanguage = {
  ...
  Retry: '重試',
  review_dash:
    'GHRetro 不只是另一個模擬器前端或 ROM 管理工具，而是一個溫柔的提醒：當那些門檻被拿開之後，復古遊戲其實可以如此單純又讓人開心。在它的核心理念裡，就是要讓那些經典遊戲變得觸手可及——好找、好玩，而且無論你在哪裡，都能輕鬆帶在身邊。底層技術再怎麼現代，骨子裡仍是最純粹的 8 位精神：選一款遊戲，按下 Start，然後盡情開玩。',
  review_jim_gray:
    '擁有一整座龐大的復古遊戲庫固然很好，但如果你把掌機忘在家裡，那些遊戲也幫不上忙。或者你想開始玩最新的「本月之作」，卻發現不知怎麼的，它根本沒裝在掌機上。GHRetro 想解決的，就是這類情境——你的整個收藏都能直接在瀏覽器裡叫出來。',
  review_korben:
    'GHRetro 可以說是一座「塞進瀏覽器分頁裡的個人復古遊戲電台」。不用安裝、不必繁瑣設定，只要打開，就能在任何裝置上存取你的復古遊戲收藏。體驗過這個平台之後，你多半會有一種感覺：好像又走進了當年那間只屬於你的電玩房……',
  review_robert_triggs:
    'GHRetro 讓管理遊戲收藏這件事，不再是例行公事，而變成一種享受。它設定簡單、操作直覺，而且在我所有裝置上都能順暢運作。終於，我可以少花點時間在調教上，把更多時間留給真正的遊戲時光。',
  Reviews: '評論',
  Rewind: '倒轉',
